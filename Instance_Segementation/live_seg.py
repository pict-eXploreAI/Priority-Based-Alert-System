...
from Instance_Segementation.mmrcnn import visualize2 as visualize
...
def run_instance_model(model,frame):
    ...
    r = results[0]
    print(r['class_ids'])
    frame,frameColors = visualize.display_instances_live(frame, r['rois'], r['masks'], r['class_ids'], class_names, r['scores'],show_mask=True)
...
if __name__ == "__main__":
    ...
    while(True):
        start = time.time()
        ret, frame = cap.read()
        frame,frameColors = run_instance_model(model,frame)
        cv2.imshow('frame',frameColors)
